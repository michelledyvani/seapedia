...
<head>
    ...
    <title>SEAPEDIA — @yield('title', 'Marketplace Indonesia')</title>
    ...
    <header class="bg-white border-b border-gray-100 sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4">
            <nav class="flex items-center justify-between h-16 gap-6">
                {{-- Logo --}}
                <a href="{{ route('home') }}" class="text-2xl font-extrabold tracking-tight shrink-0 font-heading text-neutral">
                    <span class="text-primary">SEA</span>PEDIA
                </a>

                {{-- Desktop nav links --}}
                <div class="hidden lg:flex items-center gap-1 text-sm font-medium text-gray-600">
                    <a href="{{ route('home') }}" class="px-3 py-2 rounded-lg hover:bg-base-200 hover:text-primary transition-colors">Home</a>
                    <a href="{{ route('products.index') }}" class="px-3 py-2 rounded-lg hover:bg-base-200 hover:text-primary transition-colors">Produk</a>
                    <a href="{{ route('reviews.index') }}" class="px-3 py-2 rounded-lg hover:bg-base-200 hover:text-primary transition-colors">Ulasan</a>

                    @auth
                        @if(session('active_role'))
                            <a href="{{ route('dashboard.' . session('active_role')) }}" class="px-3 py-2 rounded-lg hover:bg-base-200 hover:text-primary transition-colors">Dashboard</a>
                        @endif
                    @endauth
                </div>
                ...
                <div class="flex items-center gap-3 flex-1 lg:flex-none justify-end">
                    {{-- Search form --}}
                    <form action="{{ route('products.index') }}" method="GET" class="hidden md:flex items-center relative max-w-xs w-full">
                    ...
                    @auth
                        @if(session('active_role') === 'buyer')
                            <a href="{{ route('buyer.cart') }}" class="btn btn-ghost btn-sm btn-circle text-gray-500 hover:text-primary">
                    ...
                    @auth
                        <div class="dropdown dropdown-end">
                            ...
                            <ul tabindex="0" class="dropdown-content menu p-2 shadow-lg bg-white rounded-xl w-52 mt-2 z-50 border border-gray-100">
                                @if(session('active_role'))
                                    <li class="px-3 py-1.5"><span class="text-xs text-gray-400 uppercase tracking-wider font-semibold">{{ session('active_role') }}</span></li>
                                    <li><a href="{{ route('dashboard.' . session('active_role')) }}" class="text-sm">Dashboard</a></li>
                                @endif
                                <li><a href="{{ route('role.select') }}" class="text-sm">Ganti Role</a></li>
                    ...
                    @else
                        <div class="flex items-center gap-2">
                            <a href="{{ route('login') }}" class="btn btn-ghost btn-sm font-medium text-gray-600 hidden sm:inline-flex">Masuk</a>
                            <a href="{{ route('register') }}" class="btn btn-primary btn-sm font-semibold rounded-full px-5 shadow-sm shadow-primary/20 text-white">Daftar</a>
                        </div>
                    @endauth
                </div>
            </nav>
    ...
    <footer class="bg-white border-t border-gray-100 mt-auto">
        <div class="max-w-7xl mx-auto px-4 py-14">
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-10">
                ...
                <div class="col-span-2 md:col-span-1 lg:col-span-1">
                    <a href="{{ route('home') }}" class="text-2xl font-extrabold tracking-tight font-heading text-neutral mb-4 block">
                        <span class="text-primary">SEA</span>PEDIA
                    </a>
                ...
                <div>
                    ...
                    <ul class="space-y-2.5 text-sm text-gray-500">
                        ...
                        <li><a href="{{ route('products.index') }}" class="hover:text-primary transition-colors">Produk Baru</a></li>
                        <li><a href="{{ route('reviews.index') }}" class="hover:text-primary transition-colors">Ulasan</a></li>
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-2.5 text-sm text-gray-500">
                        <li><a href="{{ route('products.index') }}" class="hover:text-primary transition-colors">Belanja</a></li>
                        <li><a href="{{ route('register') }}" class="hover:text-primary transition-colors">Mulai Berjualan</a></li>
                        <li><a href="{{ route('register') }}" class="hover:text-primary transition-colors">Mitra Driver</a></li>
        ...
        <div class="border-t border-gray-100">
            <div class="max-w-7xl mx-auto px-4 py-5 flex flex-col md:flex-row justify-between items-center gap-3 text-xs text-gray-400">
                <p>&copy; {{ date('Y') }} PT SEAPEDIA Nusantara. Hak Cipta Dilindungi.</p>
