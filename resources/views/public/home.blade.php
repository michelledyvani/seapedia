...
{{-- ── Hero Section ─ Light gradient bg, text left, decorative shapes right ── --}}
<section class="relative overflow-hidden" style="background: linear-gradient(135deg, #f0f5ff 0%, #e8f0fe 40%, #fef9ef 100%);">
    ...
    <div class="max-w-7xl mx-auto px-4 py-16 md:py-24 relative z-10">
        <div class="max-w-2xl">
            ...
            {{-- Search bar --}}
            <form action="{{ route('products.index') }}" method="GET" class="flex items-center gap-3 mb-6 max-w-lg">
            ...
            <div class="flex gap-3">
                <a href="{{ route('products.index') }}" class="btn btn-primary btn-sm rounded-full px-5 shadow-sm shadow-primary/20 font-semibold text-white">Produk Unggulan</a>
                <a href="{{ route('register') }}" class="btn btn-ghost btn-sm rounded-full px-5 border border-gray-200 text-gray-600 hover:bg-gray-50">Mulai Berjualan</a>
            </div>
        </div>
    </div>
</section>

{{-- ── Category Row ─ Horizontal cards with icons, matching reference ── --}}
<section class="bg-white py-10 border-b border-gray-100">
    <div class="max-w-7xl mx-auto px-4">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <a href="{{ $cat[3] }}" class="category-pill group">
                ...
                <div class="min-w-0">
                    <p class="font-semibold text-neutral text-sm">{{ $cat[1] }}</p>
                    <p class="text-xs text-gray-400 truncate">{{ $cat[2] }}</p>
                </div>
            </a>
            @endforeach
        </div>
...
{{-- ── Featured Products ─ Clean card grid matching reference ── --}}
<section class="bg-base-200/40 py-16">
    <div class="max-w-7xl mx-auto px-4">
        ...
        {{-- Tab-like filters (visual only, matching reference) --}}
        <div class="flex justify-center gap-2 mb-10">
            <a href="{{ route('products.index') }}" class="btn btn-sm btn-primary rounded-full px-5 font-medium text-white shadow-sm">Unggulan</a>
            <a href="{{ route('products.index') }}" class="btn btn-sm btn-ghost rounded-full px-5 font-medium text-gray-500 hover:bg-gray-100">Terbaru</a>
            <a href="{{ route('products.index') }}" class="btn btn-sm btn-ghost rounded-full px-5 font-medium text-gray-500 hover:bg-gray-100">Terlaris</a>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
            ...
            <a href="{{ route('products.show', $product) }}" class="card-clean group overflow-hidden">
                ...
                <div class="p-4 pt-3">
                    <h3 class="text-sm font-semibold text-neutral line-clamp-2 leading-snug mb-1 group-hover:text-primary transition-colors">{{ $product->name }}</h3>
                    <p class="text-xs text-gray-400 mb-2 line-clamp-1">{{ $product->store->name ?? 'SEAPEDIA Store' }}</p>
        ...
        <div class="text-center mt-10">
            <a href="{{ route('products.index') }}" class="btn btn-outline btn-primary rounded-full px-8 font-semibold">Lihat Semua Produk →</a>
        </div>
    </div>
</section>
...
<section class="bg-white py-16">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="card-clean p-6">
                <div class="flex gap-1 mb-3">
                    @for($i=1;$i<=5;$i++)
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 {{ $i<=$review->rating ? 'text-amber-400 fill-current' : 'text-gray-200' }}" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                    @endfor
                </div>
                <p class="text-sm text-gray-600 mb-4 line-clamp-3 leading-relaxed">"{{ e($review->comment) }}"</p>
                <div class="flex items-center gap-3 pt-3 border-t border-gray-50">
                    <div class="avatar placeholder">
                        <div class="bg-primary/10 text-primary rounded-full w-9 text-sm font-bold">
                            <span>{{ strtoupper(substr($review->reviewer_name,0,1)) }}</span>
                        </div>
                    </div>
                    <div>
                        <p class="text-sm font-semibold text-neutral">{{ $review->reviewer_name }}</p>
        ...
        <div class="text-center mt-8">
            <a href="{{ route('reviews.index') }}" class="btn btn-ghost btn-sm text-primary hover:bg-primary/5 rounded-full px-6">Semua Ulasan →</a>
        </div>
    </div>
</section>
@endif
...
<section class="bg-primary py-14">
    <div class="max-w-4xl mx-auto px-4 text-center text-white">
        ...
        <a href="{{ route('register') }}" class="btn bg-white text-primary hover:bg-gray-50 border-none px-8 rounded-full font-bold shadow-lg">Daftar Gratis</a>
    </div>
</section>
@endsection
