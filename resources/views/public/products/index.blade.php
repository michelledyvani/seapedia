    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            <div>
                <h1 class="text-2xl font-extrabold text-neutral font-heading">
                    @if(request('q'))
                        Hasil pencarian: <span class="text-primary">"{{ request('q') }}"</span>
                    @else
                        Semua Produk
                    @endif
                </h1>
                <p class="text-sm text-gray-400 mt-1">Menampilkan {{ $products->count() }} dari {{ $products->total() }} produk</p>
            </div>
        ...
        @if($products->isEmpty())
            <div class="text-center py-20 bg-white rounded-2xl border border-gray-100">
                ...
                <a href="{{ route('products.index') }}" class="btn btn-primary btn-sm rounded-full px-6 text-white">Lihat Semua</a>
            </div>
        @else
                ...
                <a href="{{ route('products.show', $product) }}" class="card-clean group overflow-hidden">
                    <figure class="aspect-square bg-gray-50 p-3 relative">
                        ...
                        @if($product->stock < 5 && $product->stock > 0)
                            <div class="absolute top-4 right-4 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">Sisa {{ $product->stock }}</div>
                    ...
                    <div class="p-4 pt-3">
                        <p class="text-xs text-gray-400 mb-1 line-clamp-1">{{ $product->store ? $product->store->name : 'SEAPEDIA' }}</p>
